...
trait Sessionization extends Helper with TimeHandling {
  ...
  /**
    * It returns true iff, given the highway segment ''e1''-->''e2'' ''(gate(e1), gate(e2) is (not) in gates''.
    * @param gates set of pair of gates.
    * @param reverseTest true iff the containment test is reversed.
    * @param e1 event for segment entering
    * @param e2 event for segment exit
    * @return true iff, given the highway segment ''e1''-->''e2'' ''(gate(e1), gate(e2) is (not) in gates''.
    */
  def isSegmentMatchingGates(gates: Set[(Int, Int)] = Set(), reverseTest: Boolean = false)(e1: Event, e2: Event) =
    gates.contains((e1.gate, e2.gate))

  /**
    * Splits if the condition is met and the two events are an eligible split point.
    * @param mustSplit tests whether a segment forces a split
    * @param isPossibleSplit a test for split eligibility
    ...
    */
  def splitFuncIfEligible(mustSplit: (Event, Event) => Boolean)
                         (isPossibleSplit: (Event, Event) => Boolean)
                         (isSplit: (Event, Event) => Boolean)
                         (e1: Event, e2: Event): Boolean = {
    mustSplit(e1, e2) || (!isPossibleSplit(e1, e2) && isSplit(e1, e2))
  }
}
